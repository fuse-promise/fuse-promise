...
Developer install metadata is generated by `scripts/install-dev.sh`.

Installed developer artifacts:

- `fuse-promise/fuse-promise.h`
- `libfusepromise.so.<workspace-version>`
- `libfusepromise.so.0`
- `libfusepromise.so`
- `fuse-promise.pc`
- `fuse-promised`
- `fpctl`
- `fuse-promised.service`

The developer ABI uses soname-major `0` until the first stable ABI release.
The unversioned linker name points to `libfusepromise.so.0`, which points to
